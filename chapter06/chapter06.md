...
在训练集上训练，并且在训练过程中使用验证集检查训练成果。最后训练完成后，在测试集上检测最终效果。因为训练中多次使用过验证集，所以最终要使用新的数据集来测试，避免不准确。
...
注意，训练过程需要logits，而评估或者推理可以直接进行`argmax`获取label。因为`cross_entropy`第一个参数必须是logits，内部会进行`softmax`。如果`softmax`之后再传入会导致信息丢失，不可导，后续无法正确反向传播计算梯度
